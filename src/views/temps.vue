<template>
  <div style="position: fixed;z-index:99; border: 1px solid rgba(0,0,0,0);width: 100%;height: 25px;padding: 5px;background-color: rgba(0,0,0,0)">
    <input id="input_adcode" type="number" value="100000">
    <input id="hope_year" type="number" value="2013">
    <button onclick="day_control(-1,0,0)">上一年</button>
    <button onclick="day_control(1,0,0)">下一年</button>
    <input id="hope_month" type="number" value="1">
    <button onclick="day_control(0,-1,0)">上一月</button>
    <button onclick="day_control(0,1,0)">下一月</button>
    <input id="hope_day" type="number" value="1">
    <button onclick="day_control(0,0,-1)">上一天</button>
    <button onclick="day_control(0,0,1)">下一天</button>
    <button onclick="input_refresh()">按照输入的数据刷新地图</button>
  </div>
  <div style="float: right;position: relative; width: 50%; height: 845px;margin-top: 55px;background-color: rgba(0,0,0,0)">
  ...
  <div class="total_chose_box" style="width:100%;position: relative;background-color: rgba(0,0,0,0)">
    <button class="chose_enter" id="btn8">TEMP</button>
  </div>
  <div class="con left" style="width:50%;position:relative;background-color: rgba(0,0,0,0)">
    <div class="div_any">
      <div class="left div_any01" style="width:100%;">
        <div class="div_any_child" style="width:100%;position:relative;height: 13.125rem;">
          <div class="div_any_title"></div>
          <div id="wind" style="width:99%;position:relative;left:1%;height: 12.5rem;top:0.3125rem"></div>
        </div>
      </div>
    </div>
  </div>
...
export default {
  data() {
    return {
      colors: ['#20bf6b', '#0fb9b1', '#f7b731', '#fa8231', '#eb3b5a', '#6F1E51', '#1e90ff', '#5352ed', '#3742fa', '#3c40c6'],
      arr: ['PM2.5', 'PM10', 'SO2', 'NO2', 'CO', 'O3', 'U', 'V', 'TEMP', 'RH', 'PSFC'],
      mCharts4: null,
      city: '北京',
      year: 2013,
      month: 1,
  ...
  methods: {
    ...
    dataChange5(url) {
      ...
      $.get(url, function (data) {
        ...
        // 添加百度地图插件
        var bmap = this.mCharts4.getModel().getComponent('bmap').getBMap();
    ...
    input_refresh() {
      ...
      this.refresh_by_adcode_and_date_and_item(input_adcode, year, month, day, current_project_name)
      this.get_wind_echart_show(year, month, day)
    },
    refresh_by_adcode_and_date_and_item(adcode, year, month, day, air_item_name) {
      let geo_json_path
    ...
    getWindEchartShow(year, month, day) {
  ...
  mounted() {
    this.mCharts4 = echarts.init(document.getElementById('wind'), 'light');
    console.log(this.mCharts4);
    this.dataChange4(this.url4);
    // 初始化地图图表
    this.MyMapCharts = echarts.init(document.querySelector('#map_chart01'), this.themeName);

    // 添加点击事件监听器
    this.MyMapCharts.on("click", async (args) => {
      const adcode = args.data.adcode
      this.inputAdcode = adcode
      this.input_refresh()
    });

    // 添加双击事件监听器
    this.MyMapCharts.getZr().on("dblclick", async (args) => {
      if (args.target === undefined) {
        const adcode = this.inputAdcode
        if (adcode_info[adcode].parent !== null) {
          this.inputAdcode = adcode_info[adcode].parent
          this.inputRefresh()
        }
      }
    });
    ...
    this.getWindEchartShow(this.year, this.month, this.day);
  },
};
</script>
